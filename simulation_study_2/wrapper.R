...
wrapper <- function(index, N, gamma0, effect_size, pscore_threshold, data, path) {
  # Check cache
  if(!file.exists(dirname(path))) dir.create(dirname(path), recursive = TRUE)
  if(file.exists(path)) {
    cached_res <- read_rds(path)
    if(sum(cached_res$index == index & cached_res$N == N & cached_res$gamma0 == gamma0 & cached_res$effect_size == effect_size & cached_res$pscore_threshold == pscore_threshold) > 0) return()
  }
  else {
    cached_res <- NULL
  }

  print(glue::glue("Starting: {index} {N} {gamma0} {effect_size} {pscore_threshold}"))
  ...
  fit <- ate_bounds(
    data = data, X = c("X1", "X2", "X3"), A = "A", Y = "Y", 
