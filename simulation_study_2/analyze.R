...
results_summarized <- simulations |>
  ...
  group_by(N, gamma0, effect_size, smoothness, pscore_threshold) |>
  mutate(
...
results_summarized <- results_summarized |> left_join(
  ...
  by = c("N", "gamma0", "effect_size", "smoothness", "pscore_threshold")) |>
  mutate(
...
write_rds(results_summarized, "results/simulation_study_2_summary.rds")
...
table <- results_summarized |>
  ungroup() |>
  select(smoothness, pscore_threshold, effect_size, gamma0, N, n, ends_with("mean"), ends_with("median"), ends_with("q90"), ends_with("sd"), ends_with("coverage"), ends_with("power")) |> 
  arrange(pscore_threshold, effect_size, gamma0, N) |>
...
make_width_table <- function(target_gamma0 = 2, target_pscore_threshold = Inf) {
  table |>
    filter(gamma0 == target_gamma0, pscore_threshold == target_pscore_threshold) |>
    arrange(N, smoothness) |>
    select(N, n, smoothness, width_uniform_mean, width_onestep_mean, width_uniform_median, width_onestep_median, width_uniform_q90, width_onestep_q90, width_uniform_sd, width_onestep_sd ) |>
...
make_coverage_table <- function(target_gamma0 = 2, target_pscore_threshold = Inf) {
  table |>
    filter(gamma0 == target_gamma0, pscore_threshold == target_pscore_threshold) |>
    arrange(N, smoothness) |>
    select(N, n, smoothness, uniform_coverage, onestep_coverage, uniform_power, onestep_power) |>
    mutate_at(vars(N), remove_dups) |>
    rowwise() |>
